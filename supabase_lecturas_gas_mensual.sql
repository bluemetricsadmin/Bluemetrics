...
CREATE TABLE IF NOT EXISTS public.lecturas_mensuales_gas (
...
CREATE INDEX IF NOT EXISTS idx_lecturas_gas_anio ON public.lecturas_mensuales_gas(anio);
CREATE INDEX IF NOT EXISTS idx_lecturas_gas_mes ON public.lecturas_mensuales_gas(mes);

-- Crear trigger para actualizar updated_at automáticamente
CREATE OR REPLACE FUNCTION update_lecturas_mensuales_gas_updated_at()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trigger_update_lecturas_mensuales_gas_updated_at
    BEFORE UPDATE ON public.lecturas_mensuales_gas
    FOR EACH ROW
    EXECUTE FUNCTION update_lecturas_mensuales_gas_updated_at();

-- Comentarios para documentación
COMMENT ON TABLE public.lecturas_mensuales_gas IS 'Consumo mensual de gas del campus';
COMMENT ON COLUMN public.lecturas_mensuales_gas.anio IS 'Año de la lectura';
COMMENT ON COLUMN public.lecturas_mensuales_gas.mes IS 'Mes de la lectura';