...
export default function AddMonthlyGasReadingsPage() {
  const readablePoints = gasConsumptionPointsData.categories.flatMap(category => 
    category.points.filter(point => !point.noRead)
  )
  const readablePointIds = new Set(readablePoints.map(point => point.id))

 // Estados principales con persistencia
  const [selectedYear, setSelectedYear, clearSelectedYear] = usePersistedState('monthly_gas_selectedYear', DEFAULT_YEAR)
  const [selectedMonth, setSelectedMonth, clearSelectedMonth] = usePersistedState('monthly_gas_selectedMonth', new Date().getMonth() + 1)
  const [step, setStep, clearStep] = usePersistedState('monthly_gas_step', 1)
  
  // Estados para Excel y datos con persistencia
  const [readings, setReadings, clearReadings] = usePersistedState('monthly_gas_readings', {})
  const [excelData, setExcelData, clearExcelData] = usePersistedState('monthly_gas_excelData', null)
  
  // Estados para cálculo de consumo con persistencia
  const [previousMonthReadings, setPreviousMonthReadings] = useState(null)
  const [consumption, setConsumption, clearConsumption] = usePersistedState('monthly_gas_consumption', {})
  ...
  const [activeCategory, setActiveCategory, clearActiveCategory] = usePersistedState('monthly_gas_activeCategory', 'todos_los_puntos')
  const [showOrderModal, setShowOrderModal] = useState(false)
  const [orderedCategoriesData, setOrderedCategoriesData] = useState(() => 
    applyOrderToCategories(gasConsumptionPointsData.categories)
  )
  const [monthExists, setMonthExists] = useState(false)
  
  // Estado para seleccionar tipo de tabla destino
  const [targetTable, setTargetTable, clearTargetTable] = usePersistedState('monthly_gas_targetTable', 'ambas') // 'lectura', 'consumo' o 'ambas'
  ...
  const getMonthDateRange = (year, month) => {
    ...
    const startDate = new Date(Date.UTC(safeYear, safeMonth - 1, 1))
 ...
 // Verificar si el mes ya existe al cambiar año/mes
  ...
  useEffect(() => {
    if (!orderedCategoriesData || orderedCategoriesData.length === 0) return

    const hasActiveCategory = orderedCategoriesData.some(category => category.id === activeCategory)
    if (!hasActiveCategory) {
      setActiveCategory(orderedCategoriesData[0].id)
    }
  }, [orderedCategoriesData, activeCategory, setActiveCategory])

const checkIfMonthExists = async () => {
    try {
      const tableName = getMonthlyGasTableName()
   ...
   // Crear nuevo mes
      const createMonth = async () => {
        ...
        try {
          ...
          const monthDateRange = getMonthDateRange(selectedYear, selectedMonth)
  ...
  const handleExcelUpload = async (e) => {
    ...
    try {
     ...
     // Factores de multiplicación para puntos de gas
        ...
        category.points.forEach(point => {
          if (!point.noRead) {
            ...
            if (!isNaN(currentValue) && prevMonth) {
              const previousValue = parseFloat(prevMonth[`${point.id}`]) || 0
      ...
      if (ignoredNoRead > 0) {
        message += `. ${ignoredNoRead} puntos marcados como no lectura fueron omitidos`
      }
    ...
    const saveReadings = async () => {
      ...
      try {
        ...
        if (targetTable === 'lectura' || targetTable === 'ambas') {
          const lecturasTableName = getMonthlyGasTableName()
...
// Manejar edición de lectura y recalcular consumo
  const handleReadingChange = (pointId, value) => {
    ...
    if (previousMonthReadings && value && value.trim() !== '') {
      ...
      if (!isNaN(currentValue)) {
        const previousValue = parseFloat(previousMonthReadings[`${pointId}`]) || 0
  ...
  const downloadTemplate = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Lecturas Mensuales Gas')
    
    const totalPuntos = readablePoints.length
    
    const instrucciones = [
      { 'INSTRUCCIONES': 'Plantilla de Lecturas Mensuales de Gas - Aquanet' },
      { 'INSTRUCCIONES': '' },
    ...
    XLSX.writeFile(wb, `Plantilla_Lecturas_Mensuales_Gas_${fecha}.xlsx`)
  }

   // Calcular progreso
    const calculateProgress = () => {
      const total = readablePoints.length
      const readingKey = `${selectedYear}_${selectedMonth}`
      const completed = Object.entries(readings).filter(([key, value]) => {
        if (!key.endsWith(readingKey)) return false

        const pointId = key.slice(0, -(readingKey.length + 1))
        if (!readablePointIds.has(pointId)) return false
    ...
    <RedirectIfNotAuth>
      <div className="min-h-screen bg-background">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h1 className="text-3xl font-bold text-foreground mb-2">
                    Agregar Lecturas Mensuales de Gas                  </h1>
        ...
        <PointsOrderModal
          ...
          categories={gasConsumptionPointsData.categories}
