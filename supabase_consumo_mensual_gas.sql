-- Tabla para el CONSUMO mensual de gas del año 2026
CREATE TABLE IF NOT EXISTS public.lecturas_mensuales_gas_consumo (
...
CREATE INDEX IF NOT EXISTS idx_lecturas_gas_consumo_anio ON public.lecturas_mensuales_gas_consumo(anio);
CREATE INDEX IF NOT EXISTS idx_lecturas_gas_consumo_mes ON public.lecturas_mensuales_gas_consumo(mes);

-- Crear trigger para actualizar updated_at automáticamente
CREATE OR REPLACE FUNCTION update_lecturas_mensuales_gas_consumo_updated_at()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trigger_update_lecturas_mensuales_gas_consumo_updated_at
    BEFORE UPDATE ON public.lecturas_mensuales_gas_consumo
    FOR EACH ROW
    EXECUTE FUNCTION update_lecturas_mensuales_gas_consumo_updated_at();

-- Comentarios para documentación
COMMENT ON TABLE public.lecturas_mensuales_gas_consumo IS 'Consumo mensual de gas del campus';
COMMENT ON COLUMN public.lecturas_mensuales_gas_consumo.anio IS 'Año de la lectura';
COMMENT ON COLUMN public.lecturas_mensuales_gas_consumo.mes IS 'Mes de la lectura';